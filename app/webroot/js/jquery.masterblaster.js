...
( function( $, window, document, undefined ) {
  var pluginName = "masterblaster",
      ...
      //Note :: reset method, has been added by Akshat Goel
      methods = [ "push", "pop", "reset" ,"remove", "destroy" ],
  ...
  function MasterBlaster( $element, options ) {
    ...
    /* Added by Akshat :: We are doing this, so that we can match the style of the Add button, 
       with the application design.*/
    /*
      ToDo:: We can get the classes lists from the Settings outside, to make this plugin more useful
      Also, don't break the tests due to this addition. they might break
    */
    this.$addButton = $( "<button class='small button'><i class='icon-plus'></i>Add</button>" );
  ...
  MasterBlaster.prototype.push = function( tagName ) {
    if( this.isValid( tagName ) ) {
      //Note :: this line was added by Akshat. Reason : earlier, the
      //input field was getting cleared, before push function was called.
      //in that case, suppose if the push failed due to validation error,
      //the input field, was getting cleared.
      //to solve bug i moved the that line from there to here
      this.$input.val( "" );
  ...
  /* Added by Akshat Goel*/
  /* This function simply resets all the tags in the array
     and it also blanks the mb-input, field text
  */
  MasterBlaster.prototype.reset = function( ) {
